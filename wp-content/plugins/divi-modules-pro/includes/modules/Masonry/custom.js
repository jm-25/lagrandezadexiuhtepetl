(function($) {
	window.addEventListener("load", function() {
		window.dmpro_masonry_gallery();
		window.onresize = window.dmpro_masonry_gallery
	});
	...
	$.fn.dmpro_masonry_gallery = function(params) {
		let $this = $(this),
			resize = true;
		preventpagescroll = $this.attr("data-preventpagescroll");
		...
		let fixedContentPos = false;
		if ('true' === preventpagescroll) {
		...
		$this.magnificPopup({
			...
			callbacks: {
				open: function() {
					...
					$('body').css('margin-right', swidth + 'px')
				},
				close: function() {
